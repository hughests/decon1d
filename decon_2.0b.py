...
savemetime=[False]
path=os.path.dirname(os.path.realpath(__file__)) ##find the path where this script is placed
for filename in glob.glob(os.path.join(path, '*')):  ##replace * with more specific file denotation to just deconvolute certain files in the current folder.
	...
	for thgiu in savemetime:
		...
		leftview=-82.0 #desired left limit of graph (for view only)
		rightview=-85.0 #desired right limit of the graph (for view only)
		...
		num_of_mc_peaks=2 #number of monte carlo fits to run
